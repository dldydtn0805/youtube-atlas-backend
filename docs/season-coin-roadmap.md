...
시즌 코인은 `Top 20` 종목을 얼마나 잘 보유하고 운용했는지를 보여주는 시즌 명예 재화다.
...
4. 배포 후 `GET /api/game/coins/overview?regionCode=KR` 가 `200` 을 반환하는지 확인한다.
5. 스케줄러 사용 시 Top 20 보유 포지션에 대해 `coin_balance` 가 증가하는지 확인한다.
6. 매수/매도와 리더보드 계산이 기존 포인트 경제를 깨지 않았는지 확인한다.
...
4. Top 20 바깥 영상은 코인 생산 문구가 노출되지 않는지 확인한다.
