...
-- 1) Same-slot duplicate runs: keep the newest run per
--    (region_code, category_id, source, captured_at).
...
-- 2) Same-run duplicate snapshots: keep the newest snapshot per (run_id, video_id).
...
-- 3) Recreate protective indexes if they are missing.
