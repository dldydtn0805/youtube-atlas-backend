...
with recalculated_highlights as (
    ...
    cross join lateral (
        select array_remove(array[
            case
                when ghs.buy_rank is not null
                    and ghs.highlight_rank is not null
                    and ghs.buy_rank >= 20
                    and ghs.highlight_rank <= 1
                then 'ATLAS_SHOT'
            end,
            case
                when ghs.buy_rank is not null
                    and ghs.highlight_rank is not null
                    and ghs.buy_rank >= 50
                    and ghs.highlight_rank <= 20
                then 'SOLAR_SHOT'
            end,
            case
                when ghs.buy_rank is not null
                    and ghs.highlight_rank is not null
                    and ghs.buy_rank >= 100
                    and ghs.highlight_rank <= 50
                then 'MOONSHOT'
            end,
            case
                when ghs.profit_rate_percent is not null and ghs.profit_rate_percent >= 1000
                then 'BIG_CASHOUT'
                when ghs.profit_rate_percent is not null and ghs.profit_rate_percent >= 300
                then 'SMALL_CASHOUT'
            end,
            case
                when ghs.buy_rank is not null
                    and ghs.highlight_rank is not null
                    and ghs.buy_rank >= 150
                    and ghs.highlight_rank <= 100
                then 'SNIPE'
            end
...
with strategy_presence as (
    ...
    group by ghs.user_id, ghs.season_id
),
derived_codes as (
    select
        sp.user_id,
        ...
        unnest(array_remove(array[
            case when sp.has_snipe then 'SNIPE_SEEKER' end,
            case when sp.has_moonshot then 'MOON_SEEKER' end,
            case when sp.has_solar_shot then 'SOLAR_SEEKER' end,
            case when sp.has_atlas_shot then 'ATLAS_SEEKER' end,
            case when sp.has_snipe and sp.has_moonshot then 'MOON_FINDER' end,
            case when sp.has_moonshot and sp.has_solar_shot then 'SOLAR_FINDER' end,
            case when sp.has_solar_shot and sp.has_atlas_shot then 'ATLAS_FINDER' end,
            case when sp.has_snipe and sp.has_moonshot and sp.has_solar_shot then 'SOLAR_WALKER' end,
            case when sp.has_moonshot and sp.has_solar_shot and sp.has_atlas_shot then 'ATLAS_WALKER' end,
            case when sp.has_snipe and sp.has_moonshot and sp.has_solar_shot and sp.has_atlas_shot then 'ATLAS_SNIPER' end
        ], null)) as code
    from strategy_presence sp
)
